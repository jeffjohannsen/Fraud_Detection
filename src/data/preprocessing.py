...
# Suppress BeautifulSoup warning for URL-like HTML content
...
def load_fraud_data(filename):
    """
    Loads fraud detection data from specified file.

    Args:
        filename (string): Data filename without .json extension

    Returns:
        pd.DataFrame: Loaded fraud data
    """
    # Try processed directory first, then raw directory
    processed_path = DATA_PROCESSED / f"{filename}.json"
    raw_path = DATA_RAW / f"{filename}.json"
    ...
    if processed_path.exists():
        ...
        raise FileNotFoundError(
            f"Could not find {filename}.json in data/processed or data/raw"
        )
...
def fill_missing_values(df):
    ...
    for feature in empty_string_features:
        df[feature] = df[feature].replace("", "Unknown").astype(str)
    ...
    for feature in null_num_features:
        df[feature] = df[feature].fillna(-1).infer_objects(copy=False)
...
def aggregate_nested_features(df):
    ...
    df["num_tickets_available"] = df["ticket_types"].apply(get_tickets_available)
    df["total_ticket_value"] = df["ticket_types"].apply(get_total_ticket_value)
    df["avg_ticket_cost"] = df["total_ticket_value"] / df["num_tickets_available"]
...
def convert_html_to_text(df, html_features):
    ...
    for feature in html_features:
        df[feature] = df[feature].apply(lambda x: BeautifulSoup(x, "html.parser"))
...
def run_data_prep_pipeline(original_df):
    ...
    df = original_df.copy()
    df = create_target(df)
    # Map listed feature to avoid replace downcasting warning
    df["listed"] = df["listed"].map({"y": 1, "n": 0}).astype(int)
    df = convert_datetimes(df)
    df = fill_missing_values(df)

    df = aggregate_nested_features(df)
    df = convert_categorical_features(df)
    df["total_empty_values"] = df.map(lambda x: x in ["Unknown", -1, 0]).sum(axis=1)
...
def save_df_to_json(df, filename):
    ...
    Args:
        ...
        filename (string): Name of file (without .json extension)
    """
...
if __name__ == "__main__":
    ...
    model_ready_data = run_data_prep_pipeline(original_data)
    print(f"✓ Prepared {len(model_ready_data):,} records")
    print(f"✓ Features: {len(model_ready_data.columns)}")
    print(
        f"✓ Fraud rate: {(model_ready_data['is_fraud'] == 'Fraud').sum() / len(model_ready_data) * 100:.1f}%"
    )
